...
MODEL_DIR      = "src/rombodawg/Rombos-LLM-V2.5-Qwen-32b_Qlora_finetuned_w_wrapping/merged_fp16" 
...
def load_model():
    ...
    handling 4-bit QLoRA if requested.
    """
...
def generate_response(prompt: str, tokenizer, model) -> str:
    """
    Prepends the [INST] wrapper to prompt and generates a response.
    """
    ...
    enc = tokenizer(
        ...
        max_length=SEQ_LEN,
        padding=False,
        add_special_tokens=True,  # include <s> and </s>
    ).to(DEVICE)
